...
def find_correspondences_arclength(
    spline_func_left, spline_func_right, tck_right, num_samples=100, y_tol=2.0, refine_window=0.05
):
    """
    ...
    for i, p_left in enumerate(left_pts):
        ...
        def y_error(uu, y_target=y_target):
    ...
    if not matched_points_left:
        raise ValueError(
            "Could not find any corresponding points. Check spline overlap and y-coordinates."
        )
...
def find_correspondences(spline_func_left, spline_func_right, tck_right, num_samples=100):
    ...
    for p_left in left_points:
        ...
        def y_error_func(u_right, y_target=y_target):
...
def triangulate_and_reconstruct(
    calib_data,
    bspline_left_pts,
    bspline_right_pts,
    z_range=None,
    ...
    if matching == "arclength":
        points_l, points_r = find_correspondences_arclength(
            spline_func_left, spline_func_right, tck_right
        )
    else:
    ...
    if z_range is not None:
        ...
        if n_dropped:
            print(
                f"Dropped {n_dropped}/{len(points_3d)} triangulated points outside Z=[{z_min}, {z_max}] m"
            )
...
def visualize_reconstruction(
    bspline_left_pts, bspline_right_pts, points_3d, spline_3d_func, save_path=None
):
...
if __name__ == "__main__":
    ...
    calib_data = {
        "K1": K1,
        "D1": np.zeros(5),  # Assuming rectified, so distortion is zero
        "K2": K1,
        "D2": np.zeros(5),
        "R": np.identity(3),
    ...
    try:
        points_3d, spline_3d_func = triangulate_and_reconstruct(
            calib_data, bspline_left_pts, bspline_right_pts
        )

        # =========================================================================
        # 3. VISUALIZE AND USE THE RESULTS
        # =========================================================================
        visualize_reconstruction(bspline_left_pts, bspline_right_pts, points_3d, spline_3d_func)
