...
Lift two 2D B-splines (left/right rectified views) to a single 3D B-spline. Code: `src/dloseg/graph/bspline_3d_recon.py` + calibration helpers in `src/dloseg/graph/pipeline.py`.
...
`pipeline.get_zed_calibration(yaml_path, res)` parses the ZED factory calibration YAML (`src/dloseg/zed/calibration_data/zed_2i_cal_data.yaml`) into `K1/D1/K2/D2/R/T/P1/P2/baseline` for a chosen resolution (`'2K' | '1080p' | '720p' | 'VGA'`).

`pipeline.rectify_stereo_pair(left, right, calib, alpha)` wraps `cv2.stereoRectify` + `initUndistortRectifyMap` + `remap`. Use the **returned** `P1/P2` (printed by the function) for triangulation of rectified images — not the raw calibration ones.
